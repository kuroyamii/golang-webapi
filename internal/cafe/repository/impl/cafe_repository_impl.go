...
func (cr cafeRepositoryImpl) ReserveTable(ctx context.Context, tableID int) error {
	...
	if err != nil {
		log.Printf("ERROR Preparing Statement -> tableID: %v, error: %v", tableID, err.Error())
		return err
	}
	var res sql.Result
	res, err = stmt.ExecContext(ctx, tableID)
	if err != nil {
		log.Printf("ERROR Executing Statement -> tableID: %v, error: %v", tableID, err.Error())
		return err
	}
	var row int64
...
func (cr cafeRepositoryImpl) GetOrderByCustomerID(ctx context.Context, customerID uint64) (cafeEntity.Order, error) {
	...
	if err != nil {
		log.Printf("ERROR Querying -> customer ID: %v, error: %v", customerID, err.Error())
	...
	for rows.Next() {
		...
		if err != nil {
			log.Printf("ERROR Scanning -> customer ID: %v, error: %v", customerID, err.Error())
			return cafeEntity.Order{}, err
		}
	}
	return order, nil
...
func (cr cafeRepositoryImpl) GetOrderDetailsByOrderID(ctx context.Context, orderID uint64) (cafeEntity.OrderDetails, error) {
	...
	if err != nil {
		log.Printf("ERROR Querying -> order ID: %v, error: %v", orderID, err.Error())
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err != nil {
			log.Printf("ERROR Scanning -> order ID: %v, error: %v", orderID, err.Error())
			return nil, err
		}
...
func (cr cafeRepositoryImpl) InsertOrderDetails(ctx context.Context, orderID uint64, foodID []int) error {
	...
	if err != nil {
		log.Printf("ERROR Preparing Statement -> order ID: %v, food ID: %v, error: %v", orderID, foodID, err.Error())
		return err
	}
...
func (cr cafeRepositoryImpl) InsertRecord(ctx context.Context, foodID int, amount int) error {
	...
	if err != nil {
		log.Printf("ERROR Preparing Statement -> food ID: %v, error: %v", foodID, err.Error())
		return err
	}
	_, err = stmt.ExecContext(ctx, foodID, amount)
	if err != nil {
		log.Printf("ERROR Executing Statement -> food ID: %v, error: %v", foodID, err.Error())
		return err
	}
	return nil
}
...
func (cr cafeRepositoryImpl) GetWaiterByWaiterID(ctx context.Context, waiterID int) (cafeEntity.Waiter, error) {
	...
	if err != nil {
		log.Printf("ERROR Querying -> waiterID: %v, error: %v", waiterID, err.Error())
	...
	for rows.Next() {
		err = rows.Scan(&waiter.WaiterID, &waiter.Name)
		if err != nil {
			log.Printf("ERROR Scanning Rows ->  waiterID: %v, error: %v", waiterID, err.Error())
			return cafeEntity.Waiter{}, err
		}
	}
	return waiter, nil
...
func (cr cafeRepositoryImpl) GetFoodByFoodID(ctx context.Context, foodID int) (cafeEntity.Food, error) {
	...
	if err != nil {
		log.Printf("ERROR Querying -> foodID: %v, error: %v", foodID, err.Error())
	...
	for rows.Next() {
		...
		log.Printf("ERROR Scanning Rows ->  foodID: %v, error: %v", foodID, err.Error())
		return cafeEntity.Food{}, err
	}
	return food, nil
}
